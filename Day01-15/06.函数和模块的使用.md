...
def roll_dice(n=2):
    """摇色子"""
    total = 0  # 初始化总和为0
    for _ in range(n):  # 循环n次，表示摇n次色子
        total += randint(1, 6)  # 将每次摇到的点数累加到总和中
    return total  # 返回摇色子的总和


def add(a=0, b=0, c=0):
    """三个数相加"""
    return a + b + c  # 返回三个数的和


# 如果没有指定参数那么使用默认值摇两颗色子
print(roll_dice())  # 调用roll_dice函数，摇两颗色子并打印结果
# 摇三颗色子
print(roll_dice(3))  # 调用roll_dice函数，摇三颗色子并打印结果
print(add())  # 调用add函数，没有指定参数，使用默认值0，0，0相加，并打印结果
print(add(1))  # 调用add函数，只指定一个参数1，其他两个使用默认值0，相加并打印结果
print(add(1, 2))  # 调用add函数，指定两个参数1和2，最后一个参数使用默认值0，相加并打印结果
print(add(1, 2, 3))  # 调用add函数，指定三个参数1，2和3，相加并打印结果
# 传递参数时可以不按照设定的顺序进行传递
print(add(c=50, a=100, b=200))  # 调用add函数，按照参数名传递参数，c=50，a=100，b=200，相加并打印结果
```
...
def add(*args):
    total = 0  # 初始化总和为0
    for val in args:  # 遍历可变参数args中的每个值
        total += val  # 将每个值累加到总和中
    return total  # 返回总和


# 在调用add函数时可以传入0个或多个参数
print(add())  # 调用add函数，没有传入任何参数，返回0并打印结果
print(add(1))  # 调用add函数，传入一个参数1，返回1并打印结果
print(add(1, 2))  # 调用add函数，传入两个参数1和2，返回3并打印结果
print(add(1, 2, 3))  # 调用add函数，传入三个参数1、2和3，返回6并打印结果
print(add(1, 3, 5, 7, 9))  # 调用add函数，传入五个参数1、3、5、7和9，返回25并打印结果```
